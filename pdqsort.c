...
static void insertion_sort(void *_begin, void *_end, size_t size, cmp_func_t cmp_func)
{
    ...
    for (char *cur = begin + idx(1); cur != end; cur += idx(1)) {
        ...
        if (cmp_func(sift, sift_1) < 0) {
            memcpy(tmp, sift, size);
            do {
                memcpy(sift, sift_1, size);
                sift -= idx(1);
            } while (sift != begin && cmp_func(tmp, sift_1 -= idx(1)) < 0);
    ...
    for (char *cur = begin + idx(1); cur != end; cur += idx(1)) {
        ...
        if (cmp_func(sift, sift_1) < 0) {
            ...
            do {
                memcpy(sift, sift_1, size);
                sift -= idx(1);
            } while (cmp_func(tmp, sift_1 -= idx(1)) < 0);
            memcpy(sift, tmp, size);
        }
    }
    free(tmp);
...
bool partition_right(void *_begin, void *_end, size_t size, cmp_func_t cmp_func, char **ret_pivot)
{
    ...
    while (cmp_func(first += idx(1), pivot) < 0)
        ;

    if (first - idx(1) == begin)
        while (first < last && cmp_func(last -= idx(1), pivot) >= 0);
    else
        while (cmp_func(last -= idx(1), pivot) >= 0);
...
char *partition_left(void *_begin, void *_end, size_t size, cmp_func_t cmp_func)
{
    ...
    while (cmp_func(pivot, (last -= idx(1))) < 0)
        ;

    if (last + idx(1) == end)
        while (first < last && cmp_func(pivot, (first += idx(1))) >= 0);
    else
        while (cmp_func(pivot, (first += idx(1))) >= 0);
                  ...
                  bool leftmost)
{
    ...
    while (true) {
        ...
        if (!leftmost && cmp_func(begin - idx(1), begin) >= 0) {
            begin = partition_left(begin, end, size, cmp_func) + idx(1);
            continue;
        }

        char *pivot;
        bool already_partitioned = partition_right(begin, end, size, cmp_func, &pivot);
...
int main()
{
    ...
    pdqsort(arr, TEST_LEN, sizeof(uint64_t), cmpint64, NULL);
